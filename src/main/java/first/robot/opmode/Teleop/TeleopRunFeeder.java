...
@Teleop
public class TeleopRunFeeder extends PeriodicOpMode {
  private final Robot robot;

  /** The Robot instance is passed into the opmode via the constructor. */
  public TeleopRunFeeder(Robot robot) {
    this.robot = robot;

    robot.controller.leftBumper().onTrue(
        Command.noRequirements(coro -> robot.feeder.feederMotor.setThrottle(.25)).named("Jog Feeder plus"))
        .onFalse(Command.noRequirements(coro -> robot.feeder.feederMotor.setThrottle(0.)).named("Jog Feeder stop"));
